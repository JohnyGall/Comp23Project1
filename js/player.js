// This class is the player, in all of his or her androgynous glory

function Player(game, controls, x, y, playstate) {
        // Store variables
        this.game = game;
        ...
        this.animations.add('right', [10, 11, 12, 13, 14, 15, 16, 17], 10, 
                                                                  true);
        // jumping
...
Player.prototype.update = function() {
        ...
        // Set the variable for the player being on a slope
        this.onSlope = false;
        slopes.forEach(function(slope) {
                        this.onSlope = (slope.isOn(this) || this.onSlope);
                }, this);

        // If the player is out of bounds vertically (i.e. fell into a pit)
        // kill them. Stop the player from running offscreen horizontally.
        ...
        // Right Arrow
                ...
                // This is rightwards motion, so if we are moving to the left 
                // with a high velocity, decay that speed until we are back at
                // the default. Otherwise, it is business as usual and we go 
                // to the right at our default speed.
        ...
        // Left Arrow
                ...
                // Same as above, if we are going fast to the right, the left 
                // arrow key should slow us down. If not, then just change our 
                // speed to the default walking speed.
        ...
        // Reset the animation when the player is idle, so we don't 
        // walk in place.
        ...
        // Up Arrow
        // If the up arrow is being pushed, give the player a velocity upwards
        // as long as the player is touching the ground.
        ...
        // If the user is not holding down the up key, we make the jump decay 
        // faster than it normally would. This makes it so holding down the 
        // up arrow leads to a higher jump.
        else if (!this.controls.up.isDown && 
                 this.body.velocity.y <= this.UP_DECAY_THRESH) {
        ...
        if (this.controls.down.isDown && 
                ...
                // If we are falling already, set our speed to be the down speed
                // and let gravity add to that as we fall. It is important that 
                // this key never makes the player slower, so triggering it when
                // at a low falling speed will boost the player speed, but at 
                // the next frame this conditional will be false and so gravity 
                // will increase the player's speed.
        ...
        // V Key (speedboost) (Developer tool)
        if (this.controls.V.isDown) {
                ...
                this.body.velocity.x += 
                        ((2 * this.facingRight) - 1) * this.V_SPEED;
        }

        // If the player is trying to go offscreen, manually set them back to
        // coordinate 0, and kill their velocity. 
        if(this.x <= 0) {
                this.x += 1;
                this.body.velocity.x = 0;
        }
...
Player.prototype.kill = function() {
        // Set health to 'dead', i.e. anything less than MAX_HEALTH
